        ...
        private void ActualizarValoresFacturaSegunCantidad()
        {
            if (dtgDatos.SelectedRows.Count > 0 && !string.IsNullOrWhiteSpace(txtCantidad.Text))
            {
                ...
                var producto = new Productos
                {
                    ...
                    Descuento = Convert.ToDouble(selectedRow.Cells["Descuento"].Value),
                    Impuesto = Convert.ToDouble(selectedRow.Cells["Impuesto"].Value),
                    UnidadMedida = selectedRow.Cells["UnidadMedida"].Value.ToString(),
                    PrecioCompra = Convert.ToDouble(selectedRow.Cells["PrecioCompra"].Value),
    ...
    try
    {
...

        {
            ...
            if (productosComprados != null)
            {
                    ...
                    int nuevaExistencia = producto.Existencia - productosComprados;

                        // Construir la URL para actualizar el producto específico
                        string urlProducto = $"http://ApiBigFoodService.somee.com/api/Producto/{producto.CodigoInterno}";

                    // Actualizar solo la existencia (dejar otros campos intactos)
                    producto.Existencia = nuevaExistencia;

                    // Serializar el producto modificado y preparar el contenido JSON
                    ...
                    var contentProducto = new StringContent(jsonProducto, Encoding.UTF8, "application/json");
                        ...
                        var responseProducto = await client.PutAsync(urlProducto, contentProducto);

                    if (!responseProducto.IsSuccessStatusCode)
                    {
                        var errorContentProducto = await responseProducto.Content.ReadAsStringAsync();
                        MessageBox.Show($"Error al actualizar existencia del producto {producto.CodigoInterno}: {errorContentProducto}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        // Aquí podrías manejar cómo deshacer la operación si es necesario
                    }
                
            }
            else
            {
                MessageBox.Show("La lista de productos comprados está vacía o no se pudo obtener.", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
