    ...
    public partial class Frm_UI_Productos : Form
    {
        ...
        private async Task<bool> RegistrarProductos()
        {
            try
            {
                ...
                if (!long.TryParse(this.txtCodigoBarr.Text.Trim(), out long codBarra))
                {
                ...
                if (!int.TryParse(this.txtPrecioVenta.Text.Trim(), out int precioVenta))
                {
                    MessageBox.Show("El precio de venta no tiene el formato correcto.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                productos.PrecioVenta = precioVenta;

                if (!double.TryParse(this.txtDescuento.Text.Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out double descuento))
                {
                    MessageBox.Show("El descuento no tiene el formato correcto.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                productos.Descuento = descuento;

                if (!double.TryParse(this.txtImpuesto.Text.Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out double impuesto))
                {
                    MessageBox.Show("El impuesto no tiene el formato correcto.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                ...
                if (!double.TryParse(this.txtPrecioCompra.Text.Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out double precioCompra))
                {
                    MessageBox.Show("El precio de compra no tiene el formato correcto.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                ...
                if (!int.TryParse(this.txtExistencia.Text.Trim(), out int existencia))
                {
                    MessageBox.Show("La existencia no tiene el formato correcto.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
